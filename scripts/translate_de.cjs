...
const BATCH_SIZE = 30;
const DELAY_MS = 300; // Increase delay slightly as payloads are larger
...
const translateBatch = (texts, targetLang) => {
    return new Promise((resolve, reject) => {
        // Join with some unique delimiter? Newline is standard but sometimes gets messed up if source has newlines.
        // Our source vocab shouldn't have newlines.
        const q = texts.join('\n');
        ...
        https.get(url, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    // result[0] is array of segments.
                    // e.g. [ ["trans\n", "orig\n", ...], ... ]
                    // We need to reconstruct the list.
                    // Sometimes Google splits one line into multiple segments if it's long?
                    // But with "word\n", it usually keeps structure.

                    // Let's rely on splitting the full joined response string by newline
                    // to be safer against segmentation fragmentation?
                    // No, mapped segments are safer if one-to-one. 
                    // Actually, let's look at the output of test script:
                    // [ ["eins\n", "one\n", ...], ["zwei\n", "two\n", ...], ... ]
                    // It seems it respects the newlines as segment delimiters.
                    ...
                    // Join all and split by newline to handle cases where 1 source line became 2 segments?
                    const fullText = rawSegments.join('');
                    // Split and trim
                    const translations = fullText.split('\n');

                    // The last element might be empty string due to trailing newline
...
const processFile = async (file) => {
    ...
    let modifiedCount = 0;
    let itemsToTranslate = []; // { index: 5, text: "word" }

    // Identify items needing translation
    for (let i = 0; i < data.length; i++) {
        if (!data[i].de && data[i].en) {
    ...
    for (let i = 0; i < itemsToTranslate.length; i += BATCH_SIZE) {
        ...
        try {
            ...
            for (let j = 0; j < batch.length; j++) {
                if (translations[j]) {
                    data[batch[j].index].de = translations[j].trim();
                    modifiedCount++;
                }
            }
